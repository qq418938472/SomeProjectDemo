$(document).ready(function(){
	imgLocation();
	var dataImg = {"data":[{"src":"2.jpg"},{"src":"3.jpg"},{"src":"4.jpg"},{"src":"5.jpg"},{"src":"6.jpg"}]};	//模拟一个JSON字符串，模拟数据库		
	window.onscroll = function(){
		if (scrollside()) {	
			...
			$.each(dataImg.data,function(index,value){		//遍历数据,需要2个参数，一个是位置，一个是数值
				...
				$("<img>").attr("src","./images/plate/"+$(value).attr("src")).appendTo(a1);//动态创建一个img，需要设置他的属性，添加到content里面
...
function imgLocation(){
	...
	box.each(function(index, value){				       //遍历每一个box对象需要运行的函数,index从0开始
		console.log(index);
