$(document).ready(function(){
	imgLocation();
	var dataImg = {"data":[{"src":"11.jpg"},{"src":"9.jpg"},{"src":"10.jpg"},{"src":"8.jpg"},{"src":"7.jpg"},{"src":"6.jpg"},{"src":"5.jpg"},{"src":"4.jpg"},{"src":"12.jpg"}]}	//模拟一个JSON字符串，模拟数据库		
	window.onscroll = function(){
		...
		if (scrollside()) {	
			...
			$.each(dataImg.data,function(index,value){		//遍历数据,需要2个参数，一个是位置，一个是数值
				...
				$("<img>").attr("src","./images/magazine2/"+$(value).attr("src")).appendTo(a1);//动态创建一个img，需要设置他的属性，添加到content里面
...
function scrollside(){
	...
	var lastboxHeight = box.last().get(0).offsetTop+Math.floor(box.last().height()*2);//最后一张图片的高度加上最后一张图片的一半															
...
function imgLocation(){
	...
	var boxArr = Array();										//创建一个box数组,用来保存高度
	...
	box.each(function(index, value){							//遍历每一个box对象需要运行的函数
		...
		if (index<num) {										//判断当前的选择器的值是否小于第一行的值
			...
			boxArr[minboxIndex]+=box.eq(index).height();		//然后再重新算那个盒子的高度
		}
	});
}